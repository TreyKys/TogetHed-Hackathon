...
describe("Integro Golden Path End-to-End Test", function () {
    ...
    it("Should execute the full transaction lifecycle correctly", async function () {
        ...
        await mintTx.wait();
        expect(await assetToken.ownerOf(serialNumber)).to.equal(seller.address);
        console.log(`Mint successful.`);

        console.log("\nSTEP 2: Listing asset on Escrow...");
        await (await assetToken.connect(seller).approve(escrowAddress, serialNumber)).wait();
        console.log(`Seller approved Escrow contract.`);

        // The seller lists the asset, passing the price in 8-decimal TINYBARS.
        const listTx = await escrow.connect(seller).listAsset(assetTokenAddress, serialNumber, salePriceInTinybars);
        await listTx.wait();

        const listing = await escrow.listings(assetTokenAddress, serialNumber);
        ...
        const fundTx = await escrow.connect(buyer).fundEscrow(assetTokenAddress, serialNumber, { value: salePriceTxValue });
        await fundTx.wait();

        const listingAfterFunding = await escrow.listings(assetTokenAddress, serialNumber);
        ...
        const confirmTx = await escrow.connect(buyer).confirmDelivery(assetTokenAddress, serialNumber);
        ...
        expect(await assetToken.ownerOf(serialNumber)).to.equal(buyer.address);
        console.log(`- Ownership Verified: Buyer now owns Token ID ${serialNumber}.`);
        ...
        const finalListing = await escrow.listings(assetTokenAddress, serialNumber);
